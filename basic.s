...
get_input:
	r0 = 0x400000000 ll
	r1 = *(u64 *)(r0)	; Read number of accounts
	r0 += 8			; Seek to accounts table
L0:
	if r1 == 0 goto L1	; Next account
	r1 -= 1

	r3 = *(u8 *)(r0)	; Is duplicate account?
	r0 += 8
	if r3 == 0xFF goto L0
...
L0:
        w3 += 1
        r1 = *(u8 *)(r2)        ; Get string legnth
        if r1 == 0 goto L1      ; If no match, probably var assignment
        r2 += 1                 ; Avoid length byte
        call memcmp             ; Compare statement to input
        if r1 != 0 goto L2      ; No match, next
L3:
